...
class ATTN_UNET(keras.Model):
    def __init__(self):
        ...
        self.gamma  = layers.Layer.add_weight(self, name='gamma_u', shape=[1, ], initializer=tf.initializers.Zeros)
...
class ATTN_CNN(keras.Model):
    def __init__(self):
        super(ATTN_CNN, self).__init__()

        self.Conv1   = layers.Conv2D(filters=64, kernel_size=4, strides=2)
        self.BN1     = layers.BatchNormalization()
        self.Conv2   = layers.Conv2D(filters=128, kernel_size=4, strides=2)
        self.BN2     = layers.BatchNormalization()
        self.Conv3   = layers.Conv2D(filters=256, kernel_size=4, strides=2)
        self.BN3     = layers.BatchNormalization()
        self.Conv4   = layers.Conv2D(filters=512, kernel_size=4, strides=2)
        self.BN4     = layers.BatchNormalization()
        self.Conv5   = layers.Conv2D(filters=1, kernel_size=4, strides=1, use_bias=True)

        self.Atten1 = layers.Conv2D(filters=64,  kernel_size=1, strides=1)
        self.Atten2 = layers.Conv2D(filters=64,  kernel_size=1, strides=1)
        self.Atten3 = layers.Conv2D(filters=512, kernel_size=1, strides=1)
        self.gamma  = layers.Layer.add_weight(self, name='gamma_c', shape=[1, ], initializer=tf.initializers.Zeros)
    ...
    def call(self, inputs, training=None):
        ...
        g = self.Atten2(x)
        h = self.Atten3(x)
        f_flatten = tf.reshape(f, shape=[tf.shape(x)[0], tf.shape(x)[1] * tf.shape(x)[2], 64])
        g_flatten = tf.reshape(g, shape=[tf.shape(x)[0], tf.shape(x)[1] * tf.shape(x)[2], 64])
        h_flatten = tf.reshape(h, shape=[tf.shape(x)[0], tf.shape(x)[1] * tf.shape(x)[2], 512])
        s = tf.matmul(g_flatten, f_flatten, transpose_b=True)
        beta = tf.nn.softmax(s)
        o = tf.matmul(beta, h_flatten)
        gamma = self.gamma
        o = tf.reshape(o, shape=[tf.shape(x)[0], tf.shape(x)[1], tf.shape(x)[2], 512])
        x = gamma * o + x
...
class ATTN_CAE(keras.Model):
    def __init__(self):
        ...
        self.gamma = layers.Layer.add_weight(self, name='gamma_cae', shape=[1, ], initializer=tf.initializers.Zeros)
